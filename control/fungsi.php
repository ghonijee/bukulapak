...
function daftar($data){
    ...
    if (mysqli_fetch_assoc($cekuser)) {
        ...
        Value ('','$user','$pass','$namalkp','$kelamin','$email','$nohp','$alamat')"
        );
        return true;
    }
}
...
function upload($kd_order){
    ...
    if($eror===4){
        echo "<div class='alert alert-danger position-relative'>Silahkan Pilih Gambarnya Dahulu!</div>";
        return FALSE;
    }
    if (!in_array($ektensigambar,$ektensi)){
        echo "<div class='alert alert-danger position-relative'>File yang anda Upload bukan Gambar!</div>";
        return FALSE;
    }
    //upload
